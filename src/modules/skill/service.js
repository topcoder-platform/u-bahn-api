...
async function create (entity, auth) {
  ...
  try {
    await sequelize.transaction(async (t) => {
      const result = await dbHelper.create(Skill, entity, auth, t)
    ...
    if (newEntity) {
      helper.publishError(config.UBAHN_ERROR_TOPIC, newEntity, 'skill.create')
    }
    throw e
  }
}
...
async function patch (id, entity, auth, params) {
  ...
  try {
    await sequelize.transaction(async (t) => {
      const result = await dbHelper.update(Skill, id, entity, auth, params, t)
      newEntity = result.toJSON()
    ...
    if (newEntity) {
      helper.publishError(config.UBAHN_ERROR_TOPIC, newEntity, 'skill.update')
    }
    throw e
  }
}
...
async function remove (id, auth, params) {
  ...
  try {
    await sequelize.transaction(async (t) => {
      await dbHelper.remove(Skill, id, params, t)
      await serviceHelper.deleteRecordFromEs(id, params, resource)
    })
  } catch (e) {
    helper.publishError(config.UBAHN_ERROR_TOPIC, entity, 'skill.delete')
    throw e
  }
}
